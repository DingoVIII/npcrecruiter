...
export async function POST(request: Request) {
  try {
    ...
    if (!user) {
      const allowance = await freeAllowance(request, "quest-giver", true);
      if (!allowance.ok) return NextResponse.json({ error: allowance.error }, { status: allowance.error?.includes("used up") ? 429 : 503 });
    ...
    return NextResponse.json({ npc: parsed.npcs[0], questHook: parsed.questHook, remaining });
  } catch (error) {
